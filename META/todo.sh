...
archive()
{
	...
	echo "Questlog Updated!: $TODO_FILE archived."
    fi
}
...
replaceOrPrepend()
{
  ...
  [ -z "$todo" ] && die "Questlog Updated!: No task $item."
  ...
  if [ $TODOTXT_VERBOSE -gt 0 ]; then
    ...
    case "$action" in
      replace)
        echo "$item $todo"
        echo "Questlog Updated!: Replaced task with:"
        echo "$item $newtodo"
        ;;
      prepend)
        echo "$item $newtodo"
...
_addto() {
    ...
    if [ $TODOTXT_VERBOSE -gt 0 ]; then
        ...
        echo "$TASKNUM $input"
        echo "Questlog Updated!: $TASKNUM added."
    fi
}
...
"del" | "rm" )
    ...
    if [ -z "$3" ]; then
        ...
        if [ "$ANSWER" = "y" ]; then
            ...
            if [ $TODOTXT_VERBOSE -gt 0 ]; then
                echo "$item $DELETEME"
                echo "Questlog Updated!: $item deleted."
            fi
        else
        ...
        if [ $TODOTXT_VERBOSE -gt 0 ]; then
            echo "$item $DELETEME"
            echo "Questlog Updated!: Removed '$3' from task."
            echo "$item $newtodo"
        fi
    fi
    ;;
...
"depri" | "dp" )
	...
	if [ "$?" -eq 0 ]; then
		...
		NEWTODO=$(sed "$item!d" "$TODO_FILE")
		echo "$item $NEWTODO"
		echo "Questlog Updated!: $item deprioritized."
	    fi
	else
	    die "$errmsg"
	fi
    ...
    for item in `echo $* | tr ',' ' '`; do 
        ...
        if [ `echo $todo | grep -c "^x "` -eq 0 ] ; then
            ...
            if [ $TODOTXT_VERBOSE -gt 0 ]; then
                newtodo=$(sed "$item!d" "$TODO_FILE")
                echo "$item $newtodo"
                echo "Questlog Updated!: $item marked as done."
	    fi
        else
...
"move" | "mv" )
    ...
    if [ "$ANSWER" = "y" ]; then
        ...
        if [ $TODOTXT_VERBOSE -gt 0 ]; then
            echo "$item $MOVEME"
            echo "Questlog Updated!: $item moved from '$src' to '$dest'."
        fi
    else
...
note: PRIORITY must be anywhere from A to Z."
    ...
    if [ "$?" -eq 0 ]; then
        ...
        if [ $TODOTXT_VERBOSE -gt 0 ]; then
            NEWTODO=$(sed "$item!d" "$TODO_FILE")
            echo "$item $NEWTODO"
            echo "Questlog Updated!: $item prioritized ($newpri)."
	fi
    else
        die "$errmsg"
    fi
...
"report" )
    ...
    [ $TODOTXT_VERBOSE -gt 0 ] && echo "Questlog Updated!: Report file updated."
    cat "$REPORT_FILE"
    ;;
